...
static const char* TOPIC_ATTRIBUTES_REQUEST = "v1/devices/me/attributes/request/1";
static const char* TOPIC_ATTRIBUTES_RESPONSE = "v1/devices/me/attributes/response/+";
// Khởi tạo SPI_TFT
...
// Subscribe để nhận lệnh điều khiển relay từ ThingsBoard
      mqtt.subscribe(TOPIC_ATTRIBUTES_RESPONSE);
      Serial.println("Subscribed to attributes");
// Yêu cầu lấy giá trị relay từ ThingsBoard khi khởi động
      requestRelayState();
...
void requestRelayState() {
  ...
  doc["sharedKeys"] = "relay_control";
  
  char buffer[64];
  size_t n = serializeJson(doc, buffer, sizeof(buffer));
  mqtt.publish(TOPIC_ATTRIBUTES_REQUEST, buffer, n);
  Serial.println("Requested relay state from ThingsBoard");
}
...
void sendTelemetry() {
  ...
  bool lightLed = (digitalRead(LM393_D0_PIN) == LOW);

  Serial.println(lightLed);
  digitalWrite(LED_LIGHT, lightLed);

  // Hiển thị lên màn hình TFT
  SPI_TFT(temperature, humidity, lightLed, relayState);
  ...
  // Gửi trạng thái relay lên ThingsBoard
  char buf[256];
...
// Điều khiển relay
void controlRelay(bool newState) {
  relayState = newState;
  digitalWrite(RELAY_PIN, relayState ? HIGH : LOW);
  Serial.println("Relay set to: " + String(relayState ? "ON" : "OFF"));
  
  // Gửi trạng thái relay về ThingsBoard để confirm
  StaticJsonDocument<64> doc;
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Xử lý lệnh từ ThingsBoard
  if (strstr(topic, "attributes/response")) {
    StaticJsonDocument<256> doc;
    DeserializationError error = deserializeJson(doc, payload, length);
    
    if (!error) {
      // Kiểm tra xem có shared attributes không
      if (doc.containsKey("shared")) {
        JsonObject shared = doc["shared"];
        if (shared.containsKey("relay_control")) {
          bool newRelayState = shared["relay_control"];
          Serial.println("Received relay command: " + String(newRelayState ? "ON" : "OFF"));
...
void setup() {
  Serial.begin(115200);
  Serial.println("Start ESP32 Relay and Light Sensor");
  // Cấu hình pins
  pinMode(RELAY_PIN, OUTPUT);
  pinMode(LED_LIGHT, OUTPUT);
  pinMode(LM393_D0_PIN, INPUT);
  pinMode(LM393_A0_PIN, INPUT);
